	...
	(mach_msg_header_t *InHeadP, mach_msg_header_t *OutHeadP)
{
	...
	mig_external kern_return_t S_thread_get_special_port
		(mach_port_t thread, int which_port, mach_port_t *special_port, mach_msg_type_name_t *special_portPoly);
	...
	OutP->RetCode = S_thread_get_special_port(In0P->Head.msgh_request_port, In0P->which_port, &OutP->special_port, &special_portPoly);
	if (OutP->RetCode != KERN_SUCCESS)
		return;
