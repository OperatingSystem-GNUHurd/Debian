	       ...
	       mach_port_t *child_task, mach_msg_type_name_t *child_taskPoly)
{
  ...
  real_target_task = target_task_pi->task_port;
  ...
  *child_taskPoly = MACH_MSG_TYPE_COPY_SEND;
  ...
  debug ("num: %d, get a request from task %d, new task: %d",
	 local_call_num, real_target_task, real_child_task);
  return err;
}
...
S_task_terminate (mach_port_t target_task)
{
  ...
  debug ("num: %d, get request from task %d: %s",
	 local_call_num, target_task_pi->task_port, strerror (err));
  ports_port_deref (target_task_pi);
  return err;
}
	       ...
	       vm_size_t size, boolean_t anywhere)
{
  ...
  debug ("num: %d, request is from task %d, address: %d, size: %d, anywhere: %d, %s",
	 local_call_num, target_task_pi->task_port,
	      ...
	      vm_prot_t new_protection)
{
  ...
  debug ("num: %d, request is from task %d: %s",
	 local_call_num, target_task_pi->task_port, strerror (err));
  ports_port_deref (target_task_pi);
  return err;
}
	     ...
	     vm_offset_t *offset)
{
  ...
  debug ("num: %d, get a request from task %d: %s, object: %d",
	 local_call_num, target_task_pi->task_port,
	 strerror (err), *object_name);
  ports_port_deref (target_task_pi);
  return err;
}
			 ...
			 mach_msg_type_name_t *special_portPoly)
{
  ...
  debug ("num: %d, request is from task %d (%d, %d): %s\n",
	 local_call_num, target_task_pi->task_port,
			 ...
			 mach_port_t special_port)
{
  ...
  debug ("num: %d, get request from task %d (%d, %d): %s",
	 local_call_num, target_task_pi->task_port, which_port,
	  ...
	  vm_prot_t max_protection, vm_inherit_t inheritance)
{
  ...
  debug ("num: %d, request is from task %d, anywhere: %d: %s",
	 local_call_num, target_task_pi->task_port, anywhere, strerror (err));
  ports_port_deref (target_task_pi);
  return err;
}
		 ...
		 mach_msg_type_name_t *child_threadPoly)
{
  ...
  debug ("num: %d, get a request from task %d: thread port: %d: %s",
	 local_call_num, target_task_pi->task_port,
