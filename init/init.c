...
frob_kernel_process (void)
{
  ...
  error (0, 0, "init: get bootstrap port: %d", kbs);
  assert_perror (err);
			  ...
			  mach_port_t credential)
{
  ...
  if (!booted)
    {
      error (0, 0, "set %s as an essential task", name);
      if (!strcmp (name, "auth"))
	authinit = 1;
      ...
      if (authinit && execinit && procinit)
	{
	  ...
	  error (0, 0, "init_stdarrays");
	  init_stdarrays ();
	  error (0, 0, "frob_kernel_process");
	  frob_kernel_process ();

	  error (0, 0, "launch_system");
	  launch_system ();
